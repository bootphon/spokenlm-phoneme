...
from .model import FrameLevelCrossEntropyLoss, HuBERTPhoneme
from .utils import Records, create_mask_from_lengths, set_seed
...
def evaluation(
    ...
    for batch in tqdm(dataloader, desc="Validation", leave=False, disable=not is_main_process):
        ...
        logits_mask = create_mask_from_lengths(logit_lengths, logits.size(1))
        targets_mask = create_mask_from_lengths(target_lengths, targets.size(1))
        y_true += targets[targets_mask].tolist()
        y_pred += logits.argmax(-1)[logits_mask].tolist()
        total_loss += loss.float()
    model.train()
    return total_loss.float().item() / len(dataloader), accuracy_score(y_true, y_pred)
...
def training(cfg: Configuration) -> None:
    ...
    train_dataset = PhonemeDataset(cfg.train_manifest, cfg.train_alignment, tokenizer)
    ...
    eval_dataloader = DataLoader(
        PhonemeDataset(cfg.val_manifest, cfg.val_alignment, tokenizer),
    ...
    criterion = FrameLevelCrossEntropyLoss(tokenizer.pad_id)
    ...
    records = Records(["train/loss", "train/acc", "train/batch_time", "train/data_time", "train/grad_norm"])
    ...
    for epoch in range(starting_epoch, cfg.max_steps // len(train_dataloader) + 1):
        ...
        for inputs, targets, input_lengths, target_lengths in active_dataloader:
            ...
            logits_mask = create_mask_from_lengths(logit_lengths, logits.size(1))
            targets_mask = create_mask_from_lengths(target_lengths, targets.size(1))
            train_acc = accuracy_score(targets[targets_mask].tolist(), logits.argmax(-1)[logits_mask].tolist())
            records["train/acc"].update(train_acc, inputs.size(0))
            records["train/batch_time"].update(time.perf_counter() - tick)
            step += 1
            pbar.update()
            pbar.set_postfix(loss=records["train/loss"].avg, acc=records["train/acc"].avg)
            ...
            if step >= cfg.max_steps:
                ...
                val_loss, val_acc = evaluation(model, criterion, eval_dataloader, accelerator.is_main_process)
                accelerator.log({"val/loss": val_loss, "val/acc": val_acc}, step)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HuBERT finetuning with frame-wise phoneme classification.")
